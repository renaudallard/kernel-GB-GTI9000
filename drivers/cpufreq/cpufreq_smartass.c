...
#include <linux/moduleparam.h>
#define DRIVER_DEFAULT_NR_RUNNING_ADDR 0xc03a1340

static uint nr_running_addr = DRIVER_DEFAULT_NR_RUNNING_ADDR;
static unsigned long (*nr_running_x)(void);

module_param(nr_running_addr, uint, 0444);
MODULE_PARM_DESC(nr_running_addr, "The nr_running address (default 0xc03a1340)");
...
static void cpufreq_smartass_timer(unsigned long data)
{
   ...
   */
  if (!timer_pending(&this_smartass->timer) && nr_running_x() > 0)
...
{  
  ...
  nr_running_x = (void *) nr_running_addr;
...
MODULE_DESCRIPTION("'cpufreq_smartass'");
MODULE_LICENSE("GPL");
